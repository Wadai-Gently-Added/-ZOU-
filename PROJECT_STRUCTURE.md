...
最終更新: 2026-08-05（① UI全体(ボタン/メニュー/印刷レイアウト)の文言を漏れなくlanguage/データに移行 ② HTMLモードの表示・印刷をsandbox化したiframeに変更し、読み込んだHTML内のポップアップ等がアプリ画面ごと覆ってしまう事故を防止 ③ iframeのポインター操作を透過させピンチズーム/ドラッグをフレーム内でも効くように ④ マウスホイールでのズームを追加、±ボタンを廃止し⟲を上部バーへ移動 ⑤ タブ切替時に表示中の内容を破棄せず退避/復元するよう変更）
