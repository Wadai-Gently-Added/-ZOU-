...
## v1.2セッション: 調査結果とバグ修正
- **「.mode-tabのホバーが反応しない」問題の真相**: 長期間原因不明で保留していたが、実際には`.mode-tab`自体のCSS/JSは最初から正常だった(DevToolsでの強制hover検証、単独の再現テストファイルでも問題なく動作することを確認済み)。真の原因は別の要素で、マイSVG/マイHTMLリストの行(`.saved-item`)とグループ見出し(`.group-header`)に**hover用CSSがそもそも一行も存在しなかった**こと。他AI(Grok)の分析で特定、コードのdiffで裏取り済み。`.saved-item:hover`・`.saved-item.pinned:hover`・`.group-header:hover`・`.ctx-menu button:hover`を追加して解消(既存の`.mode-tab`と同じ方針: `rgba(94,201,181,.10〜.18)`系、`@media(hover:hover)`は付けない)
- **ピン留めしても未グループのアイテムの位置が変わらないバグを修正、あわせて表示順のルールを確定**: `js/list.js`の`renderList()`は、グループ内のアイテムは`.sort((a,b)=> (b.item.pinned?1:0) - (a.item.pinned?1:0))`でピン留め済みを自動的に先頭表示していたが、未グループのトップレベルアイテムにはこの並び替えが実装されておらず、`reconcileTopOrder()`(`js/storage.js`)が管理する保存済みの手動並び順がそのまま使われていた。表示順のルールを「①グループ(フォルダ)は常に未グループのアイテムより上に固定表示②未グループのアイテム同士はピン留めしたものを先頭にまとめる」に確定し、トップレベルの`order`配列にこの2段階のソートを追加(表示のみ、保存されてる`topOrder`自体は書き換えない。将来的にグループ優先/ピン優先を設定で切り替えられるようコード側にコメントを残してある)
- **マイSVG/マイHTMLリストのサムネイル背景が黒に近い固定色(`#101215`)だったため、黒い線/塗りのSVGが同化して見えなくなるバグを修正**: `.saved-item .thumb`の背景を、メインビューワー(`#viewerWrap.bg-checker`)と同じ考え方の市松模様(透過チェッカー)に変更
- **サムネイルにviewBoxを持たないSVG(width/heightの固定px値のみで作られたもの)を表示すると、内容が縮小されず一部しか見えなくなるバグを修正**: `js/list.js`に`normalizeThumbSvg()`を追加。サムネイル表示直前にviewBoxが無ければwidth/height属性から自動生成し、CSS側の`width:100%; height:100%`が正しく効くようwidth/height属性自体は取り除く処理を挟むようにした
- **Grok(他AI)が特定・提案した`.saved-item:hover`等4つのhoverルールを、診断時の比較用ファイルにしか反映しておらず実ファイルへの適用を忘れていたミスを修正**: `style.css`に`.saved-item:hover`・`.saved-item.pinned:hover`・`.group-header:hover`・`.ctx-menu button:hover`を実際に追加
- **サムネイルの`viewBox`補完だけでは直らないケースがあり修正**: `<svg>`のインラインstyle属性に`width:150px; height:150px;`のように直接寸法が書かれているSVGは、CSSの`.thumb svg{width:100%;height:100%;}`より優先度が高く上書きされ、`overflow:hidden`の44×44px枠に対して等倍のまま描画されて大部分が切れて見えなくなっていた。`normalizeThumbSvg()`で`width`/`height`属性だけでなく、インラインstyleの中のwidth/heightプロパティも取り除くよう修正
- **右クリックメニューを開いてる間、対象の行/グループ見出しを選択状態っぽく見せる`.ctx-active`クラスを追加**: ネイティブのEdgeなどは右クリック中も対象がハイライトされたままになる挙動に合わせた。`showContextMenu(x, y, options, triggerEl)`に4つ目の引数を追加し、渡された要素に`ctx-active`クラスを付け外しする(`closeContextMenu()`で自動的に外れる)

## 整理(リファクタ)セッション: 動作変更ゼロの清掃(2026-09-18)
※このセッションでは機能の追加・変更は一切していない(挙動は100%同一)。以下は構造整理のみ。

### 削除(デッドコード)
- `js/viewer.js`: 未使用変数 `lastMid`(ピンを持つ2指の中点を計算していただけで一度も参照されていなかった)を削除
- `style.css`: 対応する要素/クラスが存在しない未使用ルール4件を削除
  - `textarea#pasteArea`(HTML内に id="pasteArea" は存在しない。実際は `#pasteBox`)
  - `.btn.small`(JS/HTML内で使われていない)
  - `.subhead` / `.small-empty`(JS/HTML内で使われていない)

### 統合(重複除去)
- アイテムID生成 `'s' + Date.now() + Math.random().toString(36).slice(2,7)` が4箇所(viewer.js×2, list.js×1, storage.js×1)に重複 → `storage.js` に `newItemId()` として一元化し、各所から呼ぶ形に変更

### コメント追加(②未検証機能の目印)
- `js/viewer.js` wake lock欄: TODO(未検証) — iPhone 8+での常時点灯・iPad Pro表示確認待ち
- `js/print.js` printSubmenuOptions欄: TODO(未検証) — 印刷5形式の再確認待ち
- `js/list.js` renderList欄: TODO(未検証) — 件数表示の実機確認待ち
- `js/list.js` exportBackup欄: TODO(未検証) — showSaveFilePickerのPC動作未確認
- `js/print.js` escHtml: viewer.jsからも参照される共有関数である旨の注記(消すと他ファイルが壊れるため)

### 保持したもの(消していないもの)
- `.saved-item.hovered` / `.group-header.hovered` / `.ctx-active` のCSS: JSからの付与コードが現行ファイルに無いが、PROJECT_STRUCTURE.mdが仕様として言及しているため残存(将来のJS実装用)
- `onClick(id, handler)` 安全ヘルパー(viewer.js): 現時点で呼び出し箇所が無いが、再発防止の規約として明示されているため残存
- 言語ファイル11件・index.html・mode.js: 無変更
